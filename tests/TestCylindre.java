...
public class TestCylindre {
	...
	@Before
	public void setUp() throws NomVideException {
		try {
	...
	@Test
	public void testConstructeurNomParDefaut() throws NomVideException {
		Cylindre cyl3 = null;
		Cylindre cyl4 = null;
		try {
		cyl3 = new Cylindre(new Point(0.0, 0.0, 0.0), 3.0, 5, new Vecteur(0,0,1));
		cyl4 = new Cylindre(new Point(0.0, 0.0, 0.0), 3.0, 5, new Vecteur(0,0,1));
		} catch (NomVideException e) { // shouldn't happen
		}
	...
	@Test
	public void testgetPointbasesup() {

		
		Point impact = cyl1.getPointbasesup();
		// l'impact devrait être en (2.54, 4.93, 2.23):
		x = impact.getX();
		y = impact.getY();
		z = impact.getZ();
		assertEquals("Coord. selon x mauvaise", x, 0.0, TestCylindre.EPSILON);
		assertEquals("Coord. selon y mauvaise", y, 0.0, TestCylindre.EPSILON);
		assertEquals("Coord. selon z mauvaise", z, 5.0, TestCylindre.EPSILON);
	}
	
	@Test
	public void testappartientsurflong() throws NomVideException {
		Vecteur dir = new Vecteur(-1, -3.0, -2.0);
		Point origine = new Point(3.0, 7.0, 9.0);
		Rayon lum = new Rayon(dir, origine);
		...
		Point test = new Point(1.526,2.578,6.052);
		
		int imp = cyl2.appartientsurflong(test);
		
		assertEquals("mauvais resultat", imp, 1.0, TestCylindre.EPSILON);
	}
	
	
	@Test
	public void testappartientsurflong2() throws NomVideException {
		
		
		Point test = new Point(0.0,2.0,5.0);
		
		int imp = cyl1.appartientsurflong(test);
